...
const calculate = function (loanTotal, interestRatePerMou, totalMouths) {
  ...
  const getRepayPerMouObj = function () {
    for (let i = 0; i < totalMouths; i++) {
      // 等额本息第(i+1)个月需还利息
      let repayInterestPerMouAi = loanFormula.getRepayInterestPerMouAi(
        i,
        loanTotal,
        interestRatePerMou,
        totalMouths
  ...
  const totalRepayPriceAp =
    repayPerMouObj.repayPerMouObjAp.totalRepaidArrAp[
      repayPerMouObj.repayPerMouObjAp.totalRepaidArrAp.length - 1
    ]
  ...
  return {
    loanTotal: formatFloat(loanTotal, 2),
    totalInterestAi: formatFloat(totalInterestAi, 2),
    totalRepayAi: formatFloat(totalRepayAi, 2),
